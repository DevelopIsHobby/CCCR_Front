...
먼저 정할 것은 기술이 아니라 **'옛 홈페이지를 닫을 것인가'** 입니다. 한 주소는 한 곳만
가리키므로, 옛 홈페이지를 살려 두면서 같은 주소를 새 서버로 돌릴 수는 없습니다.

닫기로 했다면 이 순서로 합니다.

1. 옛 게시판 자료를 새 사이트로 옮깁니다(작업기록 '옛 자료 이관')
2. 옛 홈페이지를 닫습니다
...
5. `.env.production` 의 `SITE_URL` 을 `https://cccr.or.kr` 로 바꾸고 `systemctl restart c3r`
