  ...
  return (
    ...
    >
      ...
      <section className="relative overflow-hidden rounded-2xl bg-navy-900 px-8 py-12 lg:px-14">
        ...
        <div
          className="absolute -left-16 top-1/2 size-72 -translate-y-1/2 rounded-full bg-brand-500/20 blur-3xl"
          aria-hidden
        />
        <div className="relative grid gap-8 lg:grid-cols-[1fr_auto] lg:items-center lg:gap-16">
          <div>
            <p className="label-mono text-flame-500">Subscribe</p>
            <p className="mt-4 text-[1.4rem] font-bold leading-snug text-white lg:text-[1.7rem]">
      ...
      <section className="mt-20">
        <SectionHeading eyebrow="Back Issues" title="지난 호 보기" />
            ...
            <li key={n.vol}>
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="label-mono text-[1.15rem] font-bold tabular-nums text-brand-600">
                    {n.vol}
                  </span>
                  <span className="label-mono tabular-nums text-ink-400">{n.date}</span>
                </div>

                <p className="mt-4 text-[1.15rem] font-bold leading-snug text-navy-900 transition-colors group-hover:text-brand-600">
                  {n.title}
                </p>
                ...
                <ul className="mt-5 flex flex-wrap gap-2">
                  {n.topics.map((t) => (
                    <li
                      key={t}
                      className="rounded-full bg-surface px-3 py-1.5 text-[0.775rem] text-ink-600"
                    >
                      {t}
                    </li>
                  ))}
