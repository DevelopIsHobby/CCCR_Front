...
function footer(ref: string, token: string): string {
  /*
    ...
    전에는 부르는 쪽이 알아서 줄바꿈을 붙였는데, 한 곳만 붙이고 나머지는 빼먹어서
    메일마다 구분선이 마지막 문장에 붙었다 떨어졌다 했다. 꼬리말이 제 여백을
    가지고 있으면 부르는 쪽은 문장만 쓰면 된다.
  */
  return [
    "",
    "",
...
export function noticeReceived({ name, ref, token }: Base): MailBody {
  return {
    ...
    text:
      `${name}님, 안녕하세요.\n\n` +
      "사업공고 수신신청을 접수했습니다.\n\n" +
      "사무국에서 확인한 뒤 승인해 드립니다. 확인이 끝나면 결과를 이 주소로 " +
      "다시 알려드리겠습니다." +
      footer(ref, token),
  };
}
