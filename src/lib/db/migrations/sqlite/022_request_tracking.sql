...
--
-- 이미 들어와 있는 신청도 빈칸으로 두면 조회가 안 되므로 아래에서 채운다.
...
UPDATE notice_subscribers
   SET ref = 'NT-' || REPLACE(SUBSTR(created_at, 3, 8), '-', '') || '-' || SUBSTR('0000' || id, -4),
       lookup_token = LOWER(HEX(RANDOMBLOB(16)))
 WHERE ref = '';
UPDATE education_proposals
   SET ref = 'ED-' || REPLACE(SUBSTR(created_at, 3, 8), '-', '') || '-' || SUBSTR('0000' || id, -4),
       lookup_token = LOWER(HEX(RANDOMBLOB(16)))
 WHERE ref = '';
UPDATE room_reservations
   SET ref = 'RM-' || REPLACE(SUBSTR(created_at, 3, 8), '-', '') || '-' || SUBSTR('0000' || id, -4),
       lookup_token = LOWER(HEX(RANDOMBLOB(16)))
 WHERE ref = '';
UPDATE promo_requests
   SET ref = 'PR-' || REPLACE(SUBSTR(created_at, 3, 8), '-', '') || '-' || SUBSTR('0000' || id, -4),
       lookup_token = LOWER(HEX(RANDOMBLOB(16)))
 WHERE ref = '';
...
CREATE TABLE mail_log (
  ...
  kind       TEXT    NOT NULL,           -- notice.received · room.confirmed 처럼 무슨 메일인지
