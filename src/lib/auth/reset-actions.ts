...
/*
  ...
  없어야 한다. 한 번 쓰면 다시 못 쓰고, 세 시간이 지나면 저절로 죽는다.
*/

/*
  링크 수명.

  한 시간은 빠듯하다. 점심에 요청해 두고 회의 다녀와서 열면 이미 죽어 있다.
  링크가 죽으면 결국 사무국에 전화하게 되어, 이 기능을 만든 뜻이 없어진다.

  늘려도 위험이 크게 늘지는 않는다. 토큰은 256비트 난수라 찍어 맞힐 수 없고,
  한 번 쓰면 즉시 죽으며, 새로 요청하면 앞 링크가 무효가 된다.
*/
const VALID_SEC = 60 * 60 * 3; // 3시간
...
export async function requestReset(
  ...
  */
  ...
  return {
    ok: "가입된 주소라면 재설정 링크를 보내드렸습니다. 메일함을 확인해 주세요. 링크는 세 시간 동안만 쓸 수 있습니다.",
  };
}
