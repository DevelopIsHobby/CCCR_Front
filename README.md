...
- **아직 자리표시자**: 고유번호(`000-00-00000`), 회비 금액(`000,000원`), 회원사 명단·수치, 게시판·행사·교육 목록.
- **연혁**: 2023년만 실제 자료가 들어가 있습니다(36건). 원본 사이트에는 2009년 이전까지 연도 탭이 있으므로, `page-data.ts`의 `HISTORY` 배열에 연도를 추가하면 탭이 자동으로 늘어납니다.
