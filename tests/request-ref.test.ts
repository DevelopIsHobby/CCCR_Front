...
test("창구·접수일·순번이 그대로 드러난다", () => {
  assert.equal(makeRef("notice", 42, "2026-09-03 05:15:00"), "NT-260903-0042");
...
test("네 자리를 넘어가도 잘리지 않는다", () => {
  ...
  assert.equal(makeRef("notice", 12345, "2026-09-03 00:00:00"), "NT-260903-12345");
});
...
test("사람이 적어 넣은 번호를 너그럽게 받는다", () => {
  ...
  assert.equal(normalizeRef("nt-260903-0042"), "NT-260903-0042");
  assert.equal(normalizeRef("  NT-260903-0042  "), "NT-260903-0042");
  assert.equal(normalizeRef("NT- 260903 -0042"), "NT-260903-0042");
});
